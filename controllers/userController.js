...
router.post("/login", (req, res) => {
  ...
  User.findOne({"email": userPlainData.email,"password": hashPass}, '_id name dob email location signkey avatarImg', (err, user) => {
    if (err) {
      res.json({"success": false,"errorCode": errorCodes.serverError,"error": err})
    } else {
      if (user == null) {
...
router.post("/addavatar",(req,res) => {
  ...
  let userId = userPlainData.id;
  let imgBase64 = userPlainData.imgBase64;

  User.findByIdAndUpdate(userId,{$set:{avatarImg:imgBase64}},{ upsert: true } , (err,updatedUser) => {
    if(err){
      res.json({"success": false,"errorCode": errorCodes.serverError,"error": err});
    }else{
